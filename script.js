...
5. Die Keys MÜSSEN exakt so heißen: foodName (String), amount (Number), calories (Number), protein (Number), carbs (Number), fat (Number).`;
 ...
 const model = base64Image ? "meta-llama/llama-4-scout-17b-16e-instruct" : "openai/gpt-oss-20b";
...
function openScanner() {
  ...
  html5QrCode.start(
    ...
    alert("Kamera konnte nicht gestartet werden. Bitte erlaube den Zugriff und versuche es erneut.");
    closeScanner();
  });
}
  ...
  try {
    ...
    if (data.status === 1) {
      ...
      if (confirm(`Produkt mit Barcode ${barcode} wurde nicht gefunden. Möchtest du die Nährwerte manuell eingeben?`)) {
        tempMethod = 'Manuell';
        editingMealId = null;
        document.getElementById('modal-title').innerText = "Manuell Eintragen (Barcode)";
        document.getElementById('save-fav-container').classList.remove('hidden');
        fillResultModal('Unbekanntes Produkt', 0, 0, 0, 0, 100);
        openModal('result-modal');
      }
    }
  } catch (error) {
